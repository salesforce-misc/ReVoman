...
with ReVoman*] automating *62* steps 😲— Create persona-based users,
set up data, call the API for multiple use-cases and verify results.

link:++{pmtemplates}/pq/pq (rc).postman_collection.json++[Postman collection JSON]
...
<6> You can set up a pre- / post-hook around a Step via `hooks`, which can help as callbacks, especially for Async operations.
...
<7> It lets you be in charge of the Step Orchestration by letting you configure a pass-list of steps to ignore for failure
<8> Provide configuration to unmarshall/deserialize response JSON into strong types
...
<9> Supply your validations/assertions to be run on a step response
...
This report has everything you need to know about what happened during a step execution. You can run further assertions on top of this report
