...
*ReVoman* is an API automation tool for JVM (Java/Kotlin) from the API-first SaaS company, *Salesforce*. It reimagines API automation by letting you execute a Postman collection in a JVM program/test.
...
* Contrary to custom frameworks, almost every team uses link:https://www.postman.com/product/what-is-postman[*Postman*] for manual testing
...
`revUp` is the method to invoke to trigger the execution. You can customize the execution by supplying a configuration.
...
Here is a detailed view of one of those Step reports:
...
* A hook lets you fiddle with the execution by plugging in your code before or after a Step
* You get the stepName and the entire rundown of what happened prior to this step as parameters
* You can do things like mutate the environment, such that the subsequent steps are affected
...
* Any day, you can find an up-to-date Postman collection for every feature you need to test, right in your VCS (Git). Developers can import these templates directly from VCS for manual testing. This comes in very handy during a team blitz.
