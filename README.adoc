...
The four steps above are a real, passing run against a deterministic local http4k-backed fixture over a loopback socket — `POST` creates an object, and later steps reuse its `{id}`. Here is the test that produces it:
  ...
  final var rundown =
      ReVoman.revUp(                          // <1>
          Kick.configure()
              ...
              .dynamicEnvironment("baseUrl", api.getBaseUrl()) // <4>
              .off());
