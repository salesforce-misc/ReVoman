...
StepReport(
    ...
    envSnapshot: PostmanEnvironment<Any?> // <4>
)
----
...
<4> Snapshot of Environment at the end of each step execution. It can be compared with previous or next step environment snapshots to see what changed in this step
...
* `STANDARD` _(default)_ — Summary + all step reports with step identity, request method/URI, response status code, and failure details. No HTTP bodies.
* `VERBOSE` — Everything: HTTP headers, request/response bodies, environment snapshots per step, polling details, stack traces, and the final mutable environment.
