...
Performance results use one formal measurement platform. Only a finalized, checksum-valid
campaign with `claimEligible=true`, produced on a reviewed controlled-host profile, can support a
performance claim. Hosted canaries, direct JMH runs, standalone captures, GC/JFR investigations,
and historical reports are diagnostic and have `claimEligible=false`; they must not be described
as speedups. See the https://salesforce-misc.github.io/ReVoman/revoman/performance.html[performance evidence policy].
...
| https://salesforce-misc.github.io/ReVoman/revoman/performance.html[Performance evidence]
|===
