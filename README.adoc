...
*ReṼoman* is an API orchestration/automation tool for JVM (Java/Kotlin) from the API-first SaaS company, *Salesforce*. It re-imagines API orchestration/automation by letting you execute a Postman collection in a JVM program/test.
...
* Newman or Postman CLI are built for node and cannot be executed within a JVM. Even if you are able to run with some hacky way, there are no easy ways to measure code-coverage or assert results on your JVM stack.
...
* It has more features, like <<#_dynamic_pre_step_and_post_step_hooks>> that lets you plug in your custom JVM code in-between the execution. The <<Rundown>> is much richer in providing execution information than the link:{pmtemplates}/restfulapidev/postman-cli-reports/restful-api.dev-2025-04-14-13-16-43.json[newman/postman-cli reports]
* It's not limited to Postman templates. We have plans to support more popular templates formats like JetBrains HTTP Client, Bruno, etc. which offers versatility for consumers.
...
== API orchestration/automation with _ReṼoman_
...
<2> Captured when <<#_polling_for_async_steps,polling>> fails due to timeout or request error
<3> Captured on successful polling completion, containing attempt count, duration, and responses
<4> <<#_execution_timing_metrics,Execution timing metrics>> for each sub-step of the step procedure (PRE_REQ_JS, HTTP_REQUEST, etc.)
...
This is useful for AI agents consuming ReṼoman via protocols like MCP, or for logging and diagnostics.
...
* `STANDARD` _(default)_ — Summary + all step reports with step identity, request method/URI, response status code, <<#_execution_timing_metrics,execution timing metrics>>, and failure details. No HTTP bodies.
...
<10> <<#_dynamic_pre_step_and_post_step_hooks>>
...
* If <<#_polling_for_async_steps,Polling>> is configured for a step, it is executed after Post-Step Hooks. A `PollingFailure` is captured in the `StepReport` if polling fails.
====

[#_execution_timing_metrics]
=== Execution Timing Metrics

[.lead]
ReṼoman captures detailed execution timing metrics for each sub-step of the <<Step Procedure>>.
...
[#_dynamic_pre_step_and_post_step_hooks]
...
to qualify a step for Pre-Step/Post-Step Hook respectively. The `Predicate` enables you to write **Dynamic** hooks, which can fire for multiple qualified steps.
...
CAUTION: The recommendation is not to add too much code in <<Pre-req and Post-res scripts>>, as it's not intuitive to troubleshoot through debugging. Use it for simple operations that can be understood without debugging, and use <<#_dynamic_pre_step_and_post_step_hooks,Pre-Step /Post-Step Hooks>> for any non-trivial operations, which are intuitive to debug.
...
* Use-cases to usually mutate are in <<Pre-req and Post-res scripts>> (using `pm.environment.set()`) and <<#_dynamic_pre_step_and_post_step_hooks,Pre-Step /Post-Step Hooks>> (using the reference `rundown.mutableEnv`) during execution.
...
ReṼoman has built-in polling support through `pollingConfig()`, eliminating the need for fragile `Thread.sleep()` workarounds in <<#_dynamic_pre_step_and_post_step_hooks,Post-Step Hooks>>.
...
The learning curve is way-way-low compared to any other automation frameworks out there (like Cucumber, RestAssured, etc), as there is no proprietary syntax or template formats required for this tool. Familiarity with Postman gets you a long way in understanding this tool. Playing with the existing link:{integrationtestdir}[Integration Tests]
...
* You can add a <<#_dynamic_pre_step_and_post_step_hooks,pre-hook>> to the Step you are interested in and add a debug point inside that. This gets hit before ReṼoman fires the request in that Step
...
* You can use this information in <<#_step_picks,Step Picks>> or <<#_dynamic_pre_step_and_post_step_hooks>> to identify a particular step to execute any conditional logic
