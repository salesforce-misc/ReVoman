...
<7> You can provide a strong type for your request to be marshalled into. This shall be passed onto which executing pre-hooks
...
Here is a detailed view of one of the step reports:

image:step-report.png[Step Report]

Here is the environment key-value pairs:
...
* There may be POJO that inherit or contain legacy classes which are hard or impossible to serialize. ReVoman lets you serialize such a legacy POJO by letting you pass `typesToIgnoreForMarshalling`, where you can you filter-out these legacy classes.
...
TIP: We just saw a low-code link:{testdir}/com/salesforce/revoman/integration/core/pq/PQE2ETest.java[**E2E test**]
...
* It unmarshalls the response into Strong JVM types as per the `responseConfig` supplied and lets you run *Type-safe* validations on the strong-type and fails-fast at first failure.
