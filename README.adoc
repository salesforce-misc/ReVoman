...
you can add more _bells & whistles_ like:
...
<10> Ignore Java cert issues when firing HTTP calls
...
* `haltOnAnyFailureExcept`, `runOnlySteps`, `skipSteps` - All these accept a `predicate` of type `ExeStepPick`, which is invoked passing the current `Step` instance to decide whether to execute or skip a step.
** There are some `ExeStepPick`s bundled with ReṼoman under `ExeStepPick.PickUtils` e.g `withName`, `inFolder` etc. You can write a custom predicate of you own too.
...
* You can plug in your java code to create/generate values for environment variables which can be populated and picked-up by subsequent steps. For example, you may want some `xyzId` but you don't have a Postman collection, but instead have a Java utility to generate/create it. You can invoke the utility in a pre-hook of a step and set the value in `mutableEnv`, so the step can pick up the `{+{xyzId}+}` from there.
...
* You can add a link:#_pre_and_post_hooks[pre-hook] to the Step you are interested and add a debug point inside that. This gets hit before ReṼoman fires the request in that Step
...
* This link:CONTRIBUTING.adoc[CONTRIBUTING] doc has all the information
to set up this library on your local and get hands-on.
