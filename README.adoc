...
image:https://img.shields.io/badge/JVM-25%2B-1c1917?style=flat-square&labelColor=1c1917[JVM 25+]
...
`MockHttpServer` listens only on an ephemeral IPv4 `127.0.0.1` address. It records request
bodies in memory so tests can inspect `server.requests()` without calling a real HTTP service.
