...
http4k-backed real-wire fixture.
...
http4k-backed real-wire fixture. You can import the same metadata and manually test it through the
`Run collection` button:

image::resfulapi-dev-pm.png[]

You can automate the same with ReṼoman by supplying the metadata and environment paths. The example
below demonstrates the engine executing API metadata (serialized in the Postman V2 schema) directly
on the JVM against the local fixture:
