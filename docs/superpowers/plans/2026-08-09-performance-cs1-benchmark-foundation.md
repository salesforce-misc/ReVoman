...
- [ ] **Step 5: Implement the two-GC retained-memory protocol without future runtime types.**
  ...
  `FullGcProtocol` records aggregate `GarbageCollectorMXBean.collectionCount`, requests `System.gc()`, waits for one bounded count increase, requests a second `System.gc()`, waits for a second bounded increase, and only then reads `MemoryMXBean.heapMemoryUsage.used`. Unsupported/disabled explicit GC or either timeout invalidates the sample. `RetainedMemoryRunner` consumes caller-supplied block, target-role, fork, and stable `replicateGroup` coordinates and launches one fresh JVM for one supplied 1,000, 2,000, or 4,000 execution checkpoint; it never schedules blocks or measures counts cumulatively in one JVM. Tasks 8 and 11 schedule at least five accepted paired host blocks, supply all three checkpoint requests for each target order, and discard a rejected baseline/candidate block and all six processes from statistics. In CS1 the worker creates/drops fake weak-reference tokens to prove the protocol. CS2a replaces/adds outcomes named `ExecutionSession` and `KickExecution`; no release retained-memory claim is permitted until those real outcomes are present and all clear.
