# CS2a Native Bounded Primitives Implementation Plan
...
**Goal:** Build and verify the deterministic native copier, Ed25519 signer, and hermetic local
signature verifier that form the protocol's content-authentication boundary.

**Architecture:** The copier and signer are static, versioned, single-purpose binaries with closed
argv/FD protocols and no shell/NSS/network dependency. The Kotlin verifier performs a bounded raw
signature check before parsing any authenticated field. Checked-in JSON recipes pin every source,
compiler/JDK/dependency input and expected output inventory.

**Tech Stack:** C17/Linux syscalls, pinned static Ed25519 implementation, Clang/GCC hardening,
ASan/UBSan, Kotlin/JVM 21, Java Ed25519 provider, Gradle `installDist`, Kotest, and deterministic
binary fixtures.

## Global Constraints

- Depends only on the approved design and plan-set constraints; it does not run privileged host
  orchestration.
- Production additions are limited to the two C sources, their two recipes, the verifier source,
  and the verifier recipe named below. Test-only parsers, vectors, corpora, and harnesses may live
  under test source/resource roots.
- The copier never parses attacker-selected paths as root. It drops permanently to the sealed
  UID/GID before opening controlled-writable source bytes and uses only anchored FDs and fixed
  modes.
- The signer receives message, output, and private-key authority only through fixed validated FDs.
  It emits one canonical detached-signature frame and never reports key material.
- The verifier bounds the raw envelope, key, and signature before verifying; it parses the envelope
  only after successful domain-separated Ed25519 verification.
...
### Task 2: Implement the permanently dropped native copier

**Files:**
- Add: `docs/superpowers/benchmarks/operators/cs2a-stage-copier-v1.c`
- Add: `docs/superpowers/benchmarks/operators/cs2a-stage-copier-v1.build.json`
- Add: `src/test/kotlin/com/salesforce/revoman/benchmark/Cs2aNativeCopierContractTest.kt`
- Add: `src/test/resources/cs2a/native/copier-mutations.tsv`
- Modify: `build.gradle.kts`

**Interfaces:**
- Closed native modes: `seal-selectors`, `seal-stage`, `seal-final-handoff`, and
  `verify-gradle-seed-copy`.
- All path authority is pre-opened and mode-specific; the process receives only the exact FDs
  documented by the design, permanently sets the sealed UID/GID, clears groups/capabilities,
  applies `no_new_privs`, closes all other FDs, and then opens source children anchored beneath the
  supplied directory FD.
- Output is one bounded length-prefixed canonical inventory/result record. Any instability,
  unsupported type, link count other than one, size/count/path/depth excess, timeout, or privilege
  mismatch exits nonzero and leaves no accepted partial output.

- [ ] **Step 1: Add RED syscall and filesystem mutants**

Cover regular files, exact five-stage inventory, symlink/hardlink/FIFO/device rejection,
rename/size/hash races, short/long streams, source escape, wrong UID/GID, residual capability/group,
extra FD, retired-copier sentinel-before-drop, final-handoff schema failures, Gradle-seed copy
mismatch, quota overflow, and correct bytes with nonzero status.

```bash
./gradlew :test --tests '*Cs2aNativeCopierContractTest' \
  --rerun-tasks --no-build-cache --no-configuration-cache --console=plain
```

Expected: RED because the source, recipe, and native test tasks are absent.

- [ ] **Step 2: Add deterministic native build tasks and implement GREEN**

Add Gradle lifecycle tasks `cs2aNativeCopierDebug`, `cs2aNativeCopierAsan`,
`cs2aNativeCopierUbsan`, and `cs2aNativeCopierRelease`. The JSON recipe must pin target triple,
compiler/linker identity, complete warning/error/hardening flags, static dependencies, source digest,
expected ELF properties, and output digest/inventory. The release build must be warning-free and
must not read user compiler configuration.

- [ ] **Step 3: Run focused native verification and commit**

```bash
./gradlew cs2aNativeCopierAsan cs2aNativeCopierUbsan \
  :test --tests '*Cs2aNativeCopierContractTest' \
  --rerun-tasks --no-build-cache --no-configuration-cache --console=plain
git diff --check
git add build.gradle.kts docs/superpowers/benchmarks/operators/cs2a-stage-copier-v1.* \
  src/test/kotlin/com/salesforce/revoman/benchmark/Cs2aNativeCopierContractTest.kt \
  src/test/resources/cs2a/native/copier-mutations.tsv
git commit -m "feat: add bounded CS2a stage copier"
```

Expected: every restored mutant is rejected and both sanitizer tasks are clean.

---

### Task 3: Implement the fixed-FD native receipt signer

**Files:**
- Add: `docs/superpowers/benchmarks/operators/cs2a-receipt-signer-v1.c`
- Add: `docs/superpowers/benchmarks/operators/cs2a-receipt-signer-v1.build.json`
- Add: `src/test/kotlin/com/salesforce/revoman/benchmark/Cs2aNativeSignerContractTest.kt`
- Add: `src/test/resources/cs2a/native/signer-vectors.tsv`
- Modify: `build.gradle.kts`

**Interfaces:**
- Fixed no-argument process profile: `/dev/null` input, one bounded message FD, one write-only result
  FD, one root-only regular private-key FD, and no other descriptors.
- Exact output is the design's fixed-length signature frame; no text diagnostics enter the signed
  protocol.
- The signer validates root execution, key/message/output identity and bounds, clears environment,
  normalizes process state, signs the exact domain-separated bytes, zeroes key buffers, and exits
  nonzero on every mismatch.

- [ ] **Step 1: Add RED interoperability and misuse tests**

Use the Task 1 verifier vectors as the single interoperability oracle. Add wrong FD/type/mode,
oversize/short/extra message, substituted key, cross-domain replay, malformed framing, nonzero
status, signal/environment/process-state injection, and key-byte leakage scans.

- [ ] **Step 2: Implement the signer and pinned release recipe**

Add `cs2aNativeSignerDebug`, `cs2aNativeSignerAsan`, `cs2aNativeSignerUbsan`, and
`cs2aNativeSignerRelease`. Pin and statically link the reviewed Ed25519 implementation in the
recipe; no system crypto/provider lookup is allowed at runtime.

- [ ] **Step 3: Run signer-to-verifier GREEN and commit**

```bash
./gradlew cs2aNativeSignerAsan cs2aNativeSignerUbsan \
  :test --tests '*Cs2aNativeSignerContractTest' \
  :benchmark-driver:test --tests '*ReceiptSignatureVerifierTest' \
  --rerun-tasks --no-build-cache --no-configuration-cache --console=plain
git diff --check
git add build.gradle.kts docs/superpowers/benchmarks/operators/cs2a-receipt-signer-v1.* \
  src/test/kotlin/com/salesforce/revoman/benchmark/Cs2aNativeSignerContractTest.kt \
  src/test/resources/cs2a/native/signer-vectors.tsv
git commit -m "feat: add CS2a receipt signer"
```

Expected: signer output verifies only under the matching domain/key/message.

---

### Task 4: Pin the hermetic verifier build and primitive provenance

**Files:**
- Add: `docs/superpowers/benchmarks/operators/cs2a-local-verifier-v1.build.json`
- Add: `src/test/kotlin/com/salesforce/revoman/benchmark/Cs2aNativeBuildRecipeTest.kt`
- Add: `benchmark-driver/src/test/kotlin/com/salesforce/revoman/benchmark/driver/integrity/VerifierDistributionReproducibilityTest.kt`
- Add: `gradle/verification-metadata.xml`
- Add: `gradle.lockfile`
- Add: `benchmark-driver/gradle.lockfile`
- Add: `buildSrc/gradle.lockfile`
- Modify: `benchmark-driver/build.gradle.kts`
...
**Interfaces:**
- One aggregate gate: `./gradlew cs2aPrimitiveGate`.
- It performs two isolated release builds for copier and signer and two isolated
  `:benchmark-driver:installDist` builds for the verifier, then compares complete sorted
  path/type/mode/size/SHA-256 inventories to the reviewed recipe digests.
- The verifier recipe pins source, wrapper/build logic, dependency locks and verification metadata,
  JDK/toolchain inventory, provider, both fixed distribution launchers/mains, fixed JVM
  argv/env/FD/cwd envelope, and CAS publication modes.
- This task creates the reviewed candidate recipe and proves the hermetic mechanism. Because Plans
  2-5 still add bound Gradle gate wiring, the v1 verifier recipe is not frozen or provisionable
  here. Plan 5 Task 2 recomputes/reviews its complete build-input and expected-distribution digests
  after the last relevant build-logic edit, reruns these tests, and only then freezes v1.

- [ ] **Step 1: Add RED recipe-schema and reproducibility tests**

Reject an unknown field, missing input digest, writable output, ambient Gradle home/init script,
daemon, build/configuration cache, network access, unverified dependency, JDK/provider drift,
non-bit-identical double build, and an output differing from the independently reviewed expected
inventory.

Generate dependency locks for every verifier build/runtime configuration and strict Gradle
verification metadata, review every admitted artifact/checksum, then make the test reject a missing
lock, unverified artifact, extra repository, changed checksum, or configuration whose resolved
component set is not represented by the reviewed locks. Include the independent `buildSrc` build,
its plugins/dependencies/repositories, and reject ambient Nexus credentials, snapshot repositories,
or property-driven repository substitution from the empty-home verifier build.

- [ ] **Step 2: Implement the recipes and aggregate gate**

Use fresh empty Gradle homes and output roots, a read-only verified dependency materialization,
network-disabled Gradle, the exact wrapper and JDK, and no user/system init scripts. The test may
materialize temporary CAS roots; production publication is deferred to the local operator plan.

- [ ] **Step 3: Verify and commit Plan 1**

```bash
./gradlew cs2aPrimitiveGate \
  :test --tests '*Cs2aNative*' \
  :benchmark-driver:test --tests '*ReceiptSignatureVerifier*' \
  --tests '*Cs2aReleaseSetVerifierTest' \
  --rerun-tasks --no-build-cache --no-configuration-cache --console=plain
git add build.gradle.kts benchmark-driver/build.gradle.kts \
  ...
  gradle/verification-metadata.xml gradle.lockfile benchmark-driver/gradle.lockfile \
  buildSrc/gradle.lockfile buildSrc/build.gradle.kts buildSrc/settings.gradle.kts \
  src/test/kotlin/com/salesforce/revoman/benchmark/Cs2aNativeBuildRecipeTest.kt \
  benchmark-driver/src/test/kotlin/com/salesforce/revoman/benchmark/driver/integrity/VerifierDistributionReproducibilityTest.kt
git diff --cached --check
git diff --quiet
test -z "$(git ls-files --others --exclude-standard)"
git commit -m "test: gate CS2a native primitives reproducibly"
test -z "$(git status --porcelain --untracked-files=all)"
```

Expected: clean reproducible inventories, sanitizer-clean corpora, and a clean worktree after the
commit. The outputs are development evidence, not administrator-provisionable release artifacts.
