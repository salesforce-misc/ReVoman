  ...
  private fun revUp(
    ...
    val stepNameToReport = pmSteps.deepFlattenItems().asSequence()
      ...
      .filter {// ! TODO 24/06/23 gopala.akshintala: Regex support to filter Step Names 
        ...
        || (runOnlySteps.isNotEmpty() && (runOnlySteps.contains(stepName) || runOnlySteps.contains(stepName.substringAfterLast(FOLDER_DELIMITER))) 
        || (skipSteps.isNotEmpty() && (!skipSteps.contains(stepName) && !skipSteps.contains(stepName.substringAfterLast(FOLDER_DELIMITER))))) }
      .fold<Map<String, Any>, Map<String, StepReport>>(mapOf()) { stepNameToReport, itemWithRegex ->
        ...
        val response: Response = runCatching { httpClient(request) }.getOrElse { throwable ->
          noFailure = haltOnAnyFailureExceptForSteps.isEmpty() || haltOnAnyFailureExceptForSteps.contains(step.name) || haltOnAnyFailureExceptForSteps.contains(step.name.substringAfterLast(FOLDER_DELIMITER))
        ...
        val stepReport = if (response.status.successful) {
          ...
          if (testScriptJsResult.isFailure) {
            noFailure = haltOnAnyFailureExceptForSteps.contains(step.name) || haltOnAnyFailureExceptForSteps.contains(step.name.substringAfterLast(FOLDER_DELIMITER))
