...
data class PostmanEnvironment(private val environment: MutableMap<String, String?> = mutableMapOf()) : MutableMap<String, String?> by environment {
  ...
  fun getValuesForKeysStartingWith(vararg prefixes: String): List<String?> =
    environment.entries.asSequence().filter { prefixes.any { suffix -> it.key.startsWith(suffix) } }.map { it.value }.toList()
  ...
  fun getValuesForKeysEndingWith(vararg suffixes: String): List<String?> =
    environment.entries.asSequence().filter { suffixes.any { suffix -> it.key.endsWith(suffix) } }.map { it.value }.toList()
}
