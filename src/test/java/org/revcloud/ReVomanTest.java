...
class ReVomanTest {
  @Test
  void lasso() {
    final var itemToOutputType = Map.of(
    ...
    final var pokemon = ReVoman.lasso(pmCollectionPath, pmEnvironmentPath, itemToOutputType, dynamicEnvironment);
    
    assertThat(pokemon.itemNameToResponseWithType.size()).isEqualTo(2);
    final Class<?> allPokemonResultType = pokemon.itemNameToResponseWithType.get("All Pokemon").getSecond();
    ...
    assertThat(((Results)allPokemonResult).results.size()).isEqualTo(limit);
    
    assertThat(pokemon.environment).containsAllEntriesOf(Map.of(
