...
readonly WORKSPACE=/Users/gopala.akshintala/code-clones/work/revoman-root/.worktrees/performance-cs2a-controlled-measurement
readonly IMPLEMENTATION=f9164a66626ded5abdd738c89656a8aa4574baf2
readonly RECOVERY_REF=refs/heads/codex/performance-cs2a-lifecycle

banner "Controlled CS2a before/after measurement"

# ── Example stage: replace with your real steps ───────────────────────────
stage "Authenticate the immutable measurement boundary"
cd "$WORKSPACE"
test -z "$(git status --porcelain)"
test "$(tr -d '\r\n' <build/cs2a-implementation-sha)" = "$IMPLEMENTATION"
git merge-base --is-ancestor "$IMPLEMENTATION" HEAD
test "$(git ls-remote origin "$RECOVERY_REF" | awk '{print $1}')" = "$IMPLEMENTATION"
say "Implementation: $IMPLEMENTATION"
say "Baseline:       83f3cd70f78ad733412d10cbc8287aaabafe7aac"
say "Remote host:    gopalaaksh-wsl3"
say "The checked-in operator will prompt through dzdo directly."
warn "This is a long privileged measurement and will create an append-only evidence commit."
if ! confirm "Start the controlled A/A and before/after measurement now?"; then
  say "No remote action was started."
  exit 0
fi

stage "Run, archive, and persist the controlled attempt"
...
printf 'OPERATOR_STATUS=%s\nPERSIST_STATUS=%s\nEVIDENCE_DIR=%s\nCS2A_EVIDENCE_SHA=%s\n' \
  "$OPERATOR_STATUS" "$PERSIST_STATUS" "$EVIDENCE_DIR" "$CS2A_EVIDENCE_SHA" \
  >"$PWD/build/cs2a-controlled-result.env"
# ──────────────────────────────────────────────────────────────────────────
