...
node {
  ...
  // Repository declarations are centralized in settings for Isolated Projects. The Node plugin
  // otherwise adds a project-local Ivy repository when download=true; use the Node installation
  // already provisioned by local development and CI instead.
  download = false
}
...
kover {
  reports {
    ...
    total {
      ...
      // The honest unit-only LINE total is 86.2% (post-exclude, `./gradlew :revoman:test` only), so
      // the
...
qodana {
  projectPath.set(layout.settingsDirectory.asFile.absolutePath)
  resultsPath.set(layout.settingsDirectory.dir("build/qodana/results").asFile.absolutePath)
...
tasks.named("qodanaScan") {
  mustRunAfter(
    ":benchmark-reporting:detekt",
    ":benchmark-reporting:spotlessJava",
    ":benchmark-reporting:spotlessKotlin",
    ":benchmark-reporting:spotlessKotlinGradle",
    ":benchmarks:detekt",
    ":benchmarks:spotlessJava",
    ":benchmarks:spotlessKotlin",
    ":benchmarks:spotlessKotlinGradle",
    "detekt",
    "spotlessJava",
    "spotlessKotlin",
