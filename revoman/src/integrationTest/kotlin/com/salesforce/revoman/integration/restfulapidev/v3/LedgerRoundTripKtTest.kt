...
/**
 * End-to-end validation of the ledger warm-skip feature against a deterministic local HTTP fixture.
 * Companion to the network-free [com.salesforce.revoman.LedgerSkipE2ETest] (which uses a JDK
 * loopback server): this one proves the same cold→warm→skip cycle survives a real HTTP round-trip
 * and a real V3-loaded `sourceHash`.
 *
...
class LedgerRoundTripKtTest {
  ...
  /**
   * Cold run = LEARN: a real POST to the local fixture produces keys → they land in learnedLedger.
   */
  @Test
  fun `cold run learns producer keys from a real API call`() {
    ...
    MockHttpServer.start(api).use { server ->
      ...
      assertThat(server.requests().map { "${it.method} ${it.path}" })
        ...
        .inOrder()
      // The deterministic fixture succeeded for all 4 steps (no skip yet — EMPTY ledger).
      ...
      // V3 load computed a real, non-empty sourceHash — the precondition for warm-skip to ever
      // fire.
  ...
  /**
   * Warm run = SKIP + INJECT: with a ledger entry matching the producer's local path + sourceHash
   ...
   *
   * Because `add-object` is skipped, `objId` comes from the ledger (a fixed, non-server id). The
   * downstream PATCH/GET then call the local fixture with that ledgered id, which legitimately 404s
   * (no such fixture-side object). That's fine — it does NOT undermine what we're validating (the
   * PRODUCER step was skipped, no HTTP, value injected). So we deliberately do NOT assert
  ...
  fun `warm run with ledger skips the producer step yet injects the ledgered value`() {
    ...
    MockHttpServer.start(api).use { server ->
      ...
      // 2. Warm run: ledger says objId/productName already produced (with the local hash). The
      // library seeds `ledger.values` into the env as the lowest-precedence floor, which satisfies
      // the skip predicate's env-superset precondition — we do NOT pre-seed manually.
      ...
      // the final live env: the local fixture's downstream PATCH (called with the ledgered id)
