...
5. **Build tooling.**
   - Use the system **`gradle`**, NOT `./gradlew` — the pinned wrapper distribution may be
     unreachable behind a proxy. (`gradle :revoman:integrationTest -PincludeCoreIT --tests
     '*WfsSeedE2ETest'`.)
...
```bash
cd <revoman-root>
gradle :revoman:integrationTest -PincludeCoreIT --tests '*WfsSeedE2ETest' --console=plain
```
...
> `gradle :revoman:integrationTest -PincludeCoreIT --tests '*WfsSeedE2ETest'` (use system `gradle`, NOT
...
> seats in local SDB. See `revoman/src/integrationTest/resources/pm-templates/v3/core/wfs/wfs-seed/README.md` for the full
> runbook.
